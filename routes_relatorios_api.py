...
from models import Relatorio, FotoRelatorio, Projeto, User, RelatorioExpress, FotoRelatorioExpress, RelatorioExpressParticipante
                ...
                if not foto_info.get('id'):
                    ...
                    # Imagens já salvas no filesystem - ler arquivo e salvar bytes no banco
                        ...
                        if foto_existente:
                            ...
                            # Atualizar metadados da foto existente
                            if 'legenda' in foto_info and foto_info['legenda']:
                                foto_existente.legenda = foto_info['legenda']
                            if 'titulo' in foto_info and foto_info['titulo']:
                                foto_existente.titulo = foto_info['titulo']
                            if 'tipo_servico' in foto_info and foto_info['tipo_servico']:
                                foto_existente.tipo_servico = foto_info['tipo_servico']
...
def gerar_numero_relatorio_express():
    """Gera o próximo número sequencial para relatório express"""
    ultimo_relatorio = RelatorioExpress.query.order_by(
        RelatorioExpress.created_at.desc()
    ).first()
    
    if ultimo_relatorio and ultimo_relatorio.numero:
        # Extrair número da string EXP-001 -> 001
        try:
            numero_atual = int(ultimo_relatorio.numero.split('-')[-1])
...
# =============================================================================
# API DE AUTOSAVE PARA RELATÓRIOS EXPRESS
# =============================================================================

@app.route('/api/relatorios-express/autosave', methods=['POST'])
@csrf.exempt
@login_required
def api_autosave_express():
    """
    POST /api/relatorios-express/autosave
    
    API de autosave para relatórios express com suporte completo a imagens.
    Processa imagens temporárias e salva dados binários no banco de dados.
    
    Body (JSON):
        - relatorio_id: ID do relatório (opcional para criação)
        - empresa_nome, empresa_endereco, etc.: Dados da empresa
        - data_visita, periodo_inicio, etc.: Dados da visita
        - participantes: Array de IDs de funcionários
        - imagens: Array de objetos {temp_id, legenda, tipo_servico, local, ordem}
        
    Returns:
        JSON: {success, relatorio_id, message, imagens}
    """
    try:
        data = request.get_json()
        if not data:
        ...
        logger.info(f"📸 EXPRESS AUTOSAVE: relatorio_id={relatorio_id}, user={current_user.id}")
        logger.info(f"📸 EXPRESS AUTOSAVE: Dados recebidos: {list(data.keys())}")
        
        # 1️⃣ CRIAR NOVO RELATÓRIO EXPRESS SE NÃO EXISTIR
        if not relatorio_id:
            # Gerar número único
            ...
            # Dados da empresa
            relatorio.empresa_nome = data.get('empresa_nome')
            relatorio.empresa_endereco = data.get('empresa_endereco')
            relatorio.empresa_telefone = data.get('empresa_telefone')
            relatorio.empresa_email = data.get('empresa_email')
            relatorio.empresa_responsavel = data.get('empresa_responsavel')
            
            # Dados da visita
            if data.get('data_visita'):
                try:
                    relatorio.data_visita = datetime.fromisoformat(
                        data['data_visita'].replace('Z', '+00:00')
                    )
                except (ValueError, TypeError):
                    relatorio.data_visita = datetime.utcnow()
            ...
            relatorio.condicoes_climaticas = data.get('condicoes_climaticas')
            relatorio.temperatura = data.get('temperatura')
            
            # Localização
            relatorio.endereco_visita = data.get('endereco_visita')
            if data.get('latitude'):
                relatorio.latitude = float(data['latitude'])
            if data.get('longitude'):
                relatorio.longitude = float(data['longitude'])
            
            # Observações
            relatorio.observacoes_gerais = data.get('observacoes_gerais')
            relatorio.pendencias = data.get('pendencias')
            relatorio.recomendacoes = data.get('recomendacoes')
            
            # Checklist
            if data.get('checklist_dados'):
                if isinstance(data['checklist_dados'], str):
                    relatorio.checklist_dados = data['checklist_dados']
                else:
                    import json
                    relatorio.checklist_dados = json.dumps(data['checklist_dados'])
        ...
        # 2️⃣ ATUALIZAR RELATÓRIO EXISTENTE
            ...
            if not relatorio:
                return jsonify({
                    'success': False,
                    'error': f'Relatório express {relatorio_id} não encontrado'
                }), 404
            ...
            campos_atualizaveis = [
                ...
                'periodo_inicio', 'periodo_fim', 'condicoes_climaticas',
                'temperatura', 'endereco_visita', 'observacoes_gerais',
            ...
            if 'data_visita' in data and data['data_visita']:
                try:
                    ...
                    logger.warning(f"Erro ao processar data_visita: {e}")
            ...
            # Atualizar checklist
            if 'checklist_dados' in data:
                if isinstance(data['checklist_dados'], str):
                    relatorio.checklist_dados = data['checklist_dados']
                elif data['checklist_dados']:
                    import json
                    relatorio.checklist_dados = json.dumps(data['checklist_dados'])
            
            logger.info(f"✅ EXPRESS AUTOSAVE: Relatório {relatorio_id} atualizado")
        
        # 3️⃣ PROCESSAR PARTICIPANTES
        if 'participantes' in data and data['participantes']:
            # Remover participantes existentes
            RelatorioExpressParticipante.query.filter_by(
                relatorio_express_id=relatorio_id
            ).delete()
            
            # Adicionar novos participantes
            for funcionario_id in data['participantes']:
                participante = RelatorioExpressParticipante()
                participante.relatorio_express_id = relatorio_id
                participante.funcionario_id = funcionario_id
                db.session.add(participante)
            
            logger.info(f"✅ EXPRESS AUTOSAVE: {len(data['participantes'])} participantes atualizados")
        
        # 4️⃣ PROCESSAR IMAGENS TEMPORÁRIAS
        imagens_data = data.get('imagens', [])
        imagens_response = []
        
        logger.info(f"📸 EXPRESS AUTOSAVE: Processando {len(imagens_data)} imagens")
        
        for img_info in imagens_data:
            temp_id = img_info.get('temp_id')
            foto_id = img_info.get('id')  # ID de foto já salva
            
            # Se já tem ID, apenas atualizar metadados
            if foto_id:
                foto = FotoRelatorioExpress.query.get(foto_id)
                if foto and foto.relatorio_express_id == relatorio_id:
                    foto.legenda = img_info.get('caption', foto.legenda)
                    foto.tipo_servico = img_info.get('category', foto.tipo_servico)
                    foto.local = img_info.get('local', foto.local)
                    foto.ordem = img_info.get('ordem', foto.ordem)
                    ...
                    logger.info(f"📝 EXPRESS AUTOSAVE: Metadados da foto {foto_id} atualizados")
                continue
            
            # Processar nova imagem temporária
            if not temp_id:
                logger.warning(f"⚠️ EXPRESS AUTOSAVE: Imagem sem temp_id ignorada")
                continue
            
            # Buscar arquivo temporário
            temp_files = []
            for ext in ALLOWED_EXTENSIONS:
                temp_path = os.path.join(TEMP_UPLOAD_FOLDER, f"{temp_id}.{ext}")
                if os.path.exists(temp_path):
                    temp_files.append(temp_path)
                    break
            
            if not temp_files:
                logger.warning(f"⚠️ EXPRESS AUTOSAVE: Arquivo temporário {temp_id} não encontrado")
                continue
            ...
            # Gerar nome de arquivo definitivo
            timestamp = datetime.now().strftime('%Y%m%d_%H%M%S%f')
            extension = temp_path.rsplit('.', 1)[1]
            filename = f"express_{relatorio_id}_{timestamp}_{temp_id[:8]}.{extension}"
            ...
            foto.relatorio_express_id = relatorio_id
            foto.filename = filename
            foto.filename_original = img_info.get('filename', filename)
            foto.legenda = img_info.get('caption', '')
            foto.tipo_servico = img_info.get('category', '')
            foto.local = img_info.get('local', '')
            foto.ordem = img_info.get('ordem', 0)
            
            # SALVAR DADOS BINÁRIOS NO BANCO DE DADOS
            foto.imagem = image_bytes
            foto.imagem_hash = calcular_hash_imagem(image_bytes)
            foto.content_type = detectar_content_type(filename, image_bytes)
            foto.imagem_size = len(image_bytes)
            
            db.session.add(foto)
            db.session.flush()  # Forçar flush para obter ID
            
            logger.info(f"✅ EXPRESS AUTOSAVE: Foto {foto.id} salva - {filename}, size={len(image_bytes)} bytes, hash={foto.imagem_hash[:16]}...")
            ...
            try:
                os.remove(temp_path)
                logger.info(f"🗑️ EXPRESS AUTOSAVE: Arquivo temporário {temp_id} removido")
            except Exception as e:
                logger.warning(f"⚠️ EXPRESS AUTOSAVE: Erro ao remover temp {temp_id}: {e}")
        
        # 5️⃣ COMMIT FINAL
        db.session.commit()
        
        logger.info(f"✅ EXPRESS AUTOSAVE: Commit realizado - {len(imagens_response)} imagens processadas")
        
        return jsonify({
            'success': True,
            'message': 'AutoSave Express executado com sucesso',
            'relatorio_id': relatorio_id,
            'relatorio': {
                'id': relatorio.id,
                'numero': relatorio.numero,
                'status': relatorio.status,
                'empresa_nome': relatorio.empresa_nome
            },
        ...
        db.session.rollback()
        logger.error(f"❌ EXPRESS AUTOSAVE: Erro - {str(e)}", exc_info=True)
        return jsonify({
            'success': False,
            'error': 'Erro ao executar AutoSave Express',
            'details': str(e)
        }), 500
