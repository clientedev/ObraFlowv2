...
def admin_configuracoes():
    """Página administrativa para configurar checklists e legendas"""
    if not current_user.is_master:
    ...
    return render_template('admin/configuracoes.html', 
                         ...
                         total_legendas=total_legendas)

# API endpoints para gestão de checklists
...
def admin_create_checklist():
    ...
    try:
        ...
        existing = ChecklistTemplate.query.filter_by(nome=data['nome']).first()
        if existing:
            return jsonify({'success': False, 'error': 'Já existe um checklist com este nome'}), 400
        
        # Obter próxima ordem
        max_ordem = db.session.query(db.func.max(ChecklistTemplate.ordem)).scalar() or 0
...
@app.route('/admin/checklist/<int:id>', methods=['GET'])
@login_required
def admin_get_checklist(id):
    """Obter dados de um checklist específico"""
    if not current_user.is_master:
        return jsonify({'success': False, 'error': 'Acesso negado'}), 403
...
@app.route('/admin/checklist/<int:id>', methods=['PUT'])
@login_required
def admin_update_checklist(id):
    ...
    try:
        ...
        # Verificar duplicação de nome (exceto o próprio item)
        existing = ChecklistTemplate.query.filter(
            ChecklistTemplate.nome == data['nome'],
            ChecklistTemplate.id != id
        ).first()
        if existing:
            return jsonify({'success': False, 'error': 'Já existe um checklist com este nome'}), 400
        
        checklist.nome = data['nome']
        checklist.descricao = data.get('descricao', '')
        checklist.categoria = data.get('categoria', 'Geral')
...
@app.route('/admin/checklist/<int:id>', methods=['DELETE'])
@login_required
def admin_delete_checklist(id):
...
# API endpoints para gestão de legendas
@app.route('/admin/legenda', methods=['POST'])
@login_required 
def admin_create_legenda():
    """Criar nova legenda predefinida"""
    if not current_user.is_master:
        return jsonify({'success': False, 'error': 'Acesso negado'}), 403
...
@app.route('/admin/legenda/<int:id>', methods=['GET'])
@login_required
def admin_get_legenda(id):
    """Obter dados de uma legenda específica"""
    if not current_user.is_master:
        return jsonify({'success': False, 'error': 'Acesso negado'}), 403
...
@login_required
def admin_update_legenda(id):
    """Atualizar legenda predefinida"""
    if not current_user.is_master:
        return jsonify({'success': False, 'error': 'Acesso negado'}), 403
    ...
    try:
        ...
        # Verificar duplicação (exceto o próprio item)
        existing = LegendaPredefinida.query.filter(
...
@app.route('/admin/legenda/<int:id>', methods=['DELETE'])
@login_required
def admin_delete_legenda(id):
    """Excluir legenda predefinida"""
    if not current_user.is_master:
        return jsonify({'success': False, 'error': 'Acesso negado'}), 403
